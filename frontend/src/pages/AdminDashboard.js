...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAdminDetails = async () => {
      try {
        const response = await axios.get('http://localhost:2845/api/admin-details', {
      ...
      <div className="p-8 flex flex-col md:flex-row gap-6 justify-center items-start">
        <div className="bg-white shadow rounded-lg p-6 w-full max-w-md">
          <h2 className="text-xl font-semibold mb-4">
            Welcome, {admin ? `${admin.FirstName} ${admin.LastName}` : 'Loading...'}
          </h2>
          {admin ? (
            <div className="text-sm text-gray-700">
              ...
              <p><span className="font-medium">Status:</span> {admin.isVerified ? 'Verified' : 'Not Verified'}</p>
            </div>
          ) : (
